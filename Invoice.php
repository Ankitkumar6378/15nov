...
// }  
	...
	public function checkDublicacy($username,$lastname,$email, $password, $mobile,$address,$city)
	{
		$sql="SELECT * FROM ".$this->invoiceUserTable." WHERE email='$email'";
		$res_u = mysqli_query($this->dbConnect, $sql);

		$sql_phone="SELECT * FROM ".$this->invoiceUserTable." WHERE mobile='$mobile'";
		$res_p = mysqli_query($this->dbConnect, $sql_phone);
		...
		elseif (mysqli_num_rows($res_p) > 0) {
		  ...
		  exit();
			...
			$qr = mysqli_query($this->dbConnect,"INSERT INTO invoice_user(first_name,last_name,email, password, mobile,address,city) values('".$username."','".$lastname."','".$email."','".$password."','".$mobile."','".$address."','".$city."')"); 
			echo '<script>
